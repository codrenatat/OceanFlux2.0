    ...
    try {
        ...
        hotelsData.forEach((hotelData) => {
            ...
            cardContent.innerHTML = `
                ...
                <button type="button" class="btn btn-danger" id="deleteBtn-${hotelData._id}">
                    Borrar
                </button>
            `;
            cardHotel.appendChild(cardContent);

            const deleteBtn = document.getElementById(`deleteBtn-${hotelData._id}`);
            deleteBtn.addEventListener("click", () => {
                borrarHotel(hotelData._id, col);
            });
        });
    } catch (error) {
        const container = document.createElement("div");
